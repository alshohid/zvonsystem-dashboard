...
import type { IStreamingPerformancePoint } from "@/src/types/dashboardOverviewTypes";
...
type StreamingPerformanceChartProps = {
  data: IStreamingPerformancePoint[];
};

export default function StreamingPerformanceChart({
  data,
}: StreamingPerformanceChartProps) {
  return (
    <section className="rounded-[24px] border border-[#E7EBF7] bg-white p-5 shadow-[0_18px_45px_rgba(46,58,131,0.06)] sm:p-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h3 className="text-lg font-semibold text-[#101828]">Streaming Performance</h3>
          <p className="mt-1 text-sm text-[#777980]">Jan &ndash; Dec 2024</p>
        </div>
        <div className="flex items-center gap-2 text-sm font-medium text-[#51608C]">
          <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: DASHBOARD_COLORS.streamAccent }} />
          Streams
        </div>
      </div>
